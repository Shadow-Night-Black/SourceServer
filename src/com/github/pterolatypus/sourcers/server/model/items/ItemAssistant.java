...
import com.github.pterolatypus.sourcers.server.model.npcs.NPCHandler;
...
public class ItemAssistant {
	...
	public boolean addItem(int item, int amount) {
		...
		if ((((freeSlots() >= 1) || playerHasItem(item, 1)) && ItemDB
				.getDatabase().getItemInList(item).getStackSize() > 1)
				|| (freeSlots() > 0)) {
			for (int i = 0; i < c.playerItems.length; i++) {
				if ((c.playerItems[i] == (item + 1))
						&& ItemDB.getDatabase().getItemInList(item)
								.getStackSize() > 1 && (c.playerItems[i] > 0)) {
	...
	public String itemType(int item) {
		for (int i = 0; i < ItemO.capes.length; i++) {
			if (item == ItemO.capes[i])
				return "cape";
		}
		for (int i = 0; i < ItemO.hats.length; i++) {
			if (item == ItemO.hats[i])
				return "hat";
		}
		for (int i = 0; i < ItemO.boots.length; i++) {
			if (item == ItemO.boots[i])
				return "boots";
		}
		for (int i = 0; i < ItemO.gloves.length; i++) {
			if (item == ItemO.gloves[i])
				return "gloves";
		}
		for (int i = 0; i < ItemO.shields.length; i++) {
			if (item == ItemO.shields[i])
				return "shield";
		}
		for (int i = 0; i < ItemO.amulets.length; i++) {
			if (item == ItemO.amulets[i])
				return "amulet";
		}
		for (int i = 0; i < ItemO.arrows.length; i++) {
			if (item == ItemO.arrows[i])
				return "arrows";
		}
		for (int i = 0; i < ItemO.rings.length; i++) {
			if (item == ItemO.rings[i])
				return "ring";
		}
		for (int i = 0; i < ItemO.body.length; i++) {
			if (item == ItemO.body[i])
				return "body";
		}
		for (int i = 0; i < ItemO.legs.length; i++) {
			if (item == ItemO.legs[i])
				return "legs";
		}
		return "weapon";
	}
		...
		for (int i = 0; i < c.playerEquipment.length; i++) {
			if (c.playerEquipment[i] > -1) {
				for (int j = 0; j < Config.ITEM_LIMIT; j++) {
					if (ItemDB.getDatabase().getItemInList(i) != null) {
						if (ItemDB.getDatabase().getItemInList(i).getItemID() == c.playerEquipment[i]) {
							for (int k = 0; k < c.playerBonus.length; k++) {
								c.playerBonus[k] += ItemDB.getDatabase()
										.getItemInList(j).getBonuses()[k];
							}
							break;
						}
					}
	...
	public boolean wearItem(int wearID, int slot) {
		...
		if (c.playerItems[slot] == (wearID + 1)) {
			...
			targetSlot = ItemO.targetSlots[wearID];
			/*
			...
			if (slot >= 0 && wearID >= 0) {
				...
				if (toEquip == toRemove + 1 && ItemO.itemStackable[toRemove]) {
	...
	public boolean bankItem(int itemID, int fromSlot, int amount) {
		...
		if (!ItemO.itemIsNote[c.playerItems[fromSlot] - 1]) {
			if (c.playerItems[fromSlot] <= 0) {
				return false;
			}
			if (ItemDB.getDatabase().getItemInList(c.playerItems[fromSlot] - 1)
					.getStackSize() > 1
		...
		} else if (ItemO.itemIsNote[c.playerItems[fromSlot] - 1]
				&& !ItemO.itemIsNote[c.playerItems[fromSlot] - 2]) {
			if (c.playerItems[fromSlot] <= 0) {
				return false;
			}
	...
	public void fromBank(int itemID, int fromSlot, int amount) {
		if (amount > 0) {
			if (c.bankItems[fromSlot] > 0) {
				if (!c.takeAsNote) {
					if (ItemO.itemStackable[c.bankItems[fromSlot] - 1]) {
						...
						&& ItemO.itemIsNote[c.bankItems[fromSlot]]) {
					if (c.bankItemsN[fromSlot] > amount) {
	...
	public boolean isStackable(int itemID) {
		return ItemO.itemStackable[itemID];
	}
	...
	public String getItemName(int ItemID) {
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (ItemDB.getDatabase().getItemInList(i) != null) {
				if (ItemDB.getDatabase().getItemInList(i).getItemID() == ItemID) {
					return ItemDB.getDatabase().getItemInList(i).getItemName();
				}
			}
		}
		return "Unarmed";
	...
	public int getItemId(String itemName) {
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (ItemDB.getDatabase().getItemInList(i) != null) {
				if (ItemDB.getDatabase().getItemInList(i).getItemName()
						.equalsIgnoreCase(itemName)) {
					return ItemDB.getDatabase().getItemInList(i).getItemID();
				}
			}
		}
		return -1;
	...
	public int getUnnotedItem(int ItemID) {
		...
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (ItemDB.getDatabase().getItemInList(i).getItemID() == ItemID) {
				NotedName = ItemDB.getDatabase().getItemInList(i).getItemName();
			}
		}
		for (int i = 0; i < Config.ITEM_LIMIT; i++) {
			if (ItemDB.getDatabase().getItemInList(i).getItemName()
					.equals(NotedName)) {
				if (ItemDB.getDatabase().getItemInList(i).getDescription()
						.startsWith("Swap this note at any bank for a") == false) {
					NewID = ItemDB.getDatabase().getItemInList(i).getItemID();
					break;
				}
			}
		}
